  ...
  const save = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Visitor details were saved.",
      });
    } catch (error) {
        ...
        <div className="space-y-6 overflow-y-auto p-5">
          <div className="flex items-center gap-3">
            <div className="relative">
              <img
                src={`https://api.dicebear.com/9.x/notionists-neutral/svg?seed=${user.id}`}
                alt=""
                className="h-14 w-14 rounded-full bg-slate-100"
              />
              <span
                className={`absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white dark:border-slate-900 ${isOnline ? "bg-emerald-500" : "bg-slate-400"}`}
              />
            </div>
                ...
                >
                  ...
                  {isEditing ? (
                    ...
                    <span className="mt-1 flex items-center gap-2 text-sm font-normal">
                      ...
                      <a
                        href={`tel:${user.phone}`}
                        className="rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-primary dark:hover:bg-slate-800"
                      ...
                      {whatsAppUrl && (
                        <a
                          ...
                          aria-label={`Message ${user.phone} on WhatsApp`}
                        >
                    ...
                    <span className="mt-1 block wrap-break-word text-sm font-normal text-slate-700 dark:text-slate-200">
                      {(user[field] as string) || "Not provided"}
                    </span>
                  )}
                </label>
              ))}
          ...
          <section>
            ...
            <div className="space-y-2">
              <button
                ...
                className="flex w-full items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-left text-sm font-medium hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800"
              >
              ...
              <button
                ...
                className="flex w-full items-center gap-2 rounded-xl border border-red-200 px-3 py-2.5 text-left text-sm font-medium text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-45 dark:border-red-900/70 dark:hover:bg-red-950/30"
              >
              ...
              <button
                ...
                title={
                  !canDeleteHistory
                    ? "End every active chat before deleting this visitor"
                    : undefined
                }
                className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-medium text-red-600/75 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-45"
              >
                <Trash2 className="h-3.5 w-3.5" />
                Delete visitor profile and all history
              </button>
              {!canDeleteHistory && (
                <p className="text-xs leading-relaxed text-slate-500">
                  End every active chat before deleting this visitor or their
                  history.
                </p>
              )}
            </div>
